...
export class ByteMatrix {
    ...
    det() {
        ...
        if (DeterminantCache.has(cacheKey)) return DeterminantCache.get(cacheKey);

        let detValue;
        if (this.#domain.id === 0) detValue = this.#detGFp();
        else if (this.#domain.id === 1) detValue = this.#detZ();
        else throw new Error('Cannot take determinant from: ' + typeof(this));

        DeterminantCache.set(cacheKey, detValue);
        return detValue;
    }
