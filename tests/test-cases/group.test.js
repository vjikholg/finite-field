...
test ('Generate Z/2Z additively'), () => {
    let generators = []; 
    generators.push(-1); 
    let group = new FiniteGroup(generators, Number.MAX_SAFE_INTEGER); // Z/2Z generated by GL(Z), so "infinite order"; 


    expect(group.order == 2); 
    expect(group.elems.contains(1)); 
    expect(group.elems.contains(-1)); 
